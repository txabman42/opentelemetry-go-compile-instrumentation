...
type InstStructRule struct {
	InstBaseRule
	Struct   string             `json:"struct"`    // The type name of the struct to be instrumented
	NewField []*InstStructField `json:"new_field"` // The new fields to be added
}
...
func (r *InstStructRule) validate() error {
	// if err := r.InstBaseRule.validate(); err != nil {
	// 	return err
	// }
