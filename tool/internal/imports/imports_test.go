...
func TestCollectImportPaths(t *testing.T) {
	tests := []struct {
		name     string
		root     *dst.File
		expected map[string]string
		...
		{
			name: "mixed imports - blank, dot, and aliased",
			root: &dst.File{
				Decls: []dst.Decl{
					&dst.GenDecl{
						Tok: token.IMPORT,
		...
		{
			name: "non-import decls are ignored",
			root: &dst.File{
				Decls: []dst.Decl{
					&dst.GenDecl{
						Tok: token.VAR,
		...
		{
			name: "import spec with nil path is skipped",
			root: &dst.File{
				Decls: []dst.Decl{
					&dst.GenDecl{
						Tok: token.IMPORT,
						Specs: []dst.Spec{
							&dst.ImportSpec{Path: nil},
							&dst.ImportSpec{Path: &dst.BasicLit{Value: `"os"`}},
						},
					},
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := CollectImportPaths(tt.root)
			assert.Equal(t, tt.expected, result)
		})
	}
}
